"""An implementation of a wheelfile from https://github.com/pypa/packaging/pull/805.

All credit goes to the authors therein, specifically Alex Grönholm (GitHub: agronholm).
"""
...
class WheelMetadata(NamedTuple):
    ...
    @classmethod
    def from_filename(cls, fname: str) -> Self:
...
class WheelArchiveFile:
    def __init__(self, fp: IO[bytes], arcname: str, record_entry: WheelRecordEntry | None):
        self._fp = fp
...
class WheelReader:
    ...
    def iterate_contents(self) -> Iterator[WheelContentElement]:
        for fname, entry in self._record_entries.items():
            with self._zip.open(fname, "r") as stream:
                yield WheelContentElement(PurePath(fname), entry.hash_value, entry.filesize, stream)
    ...
    def open(self, archive_name: str) -> WheelArchiveFile:
        ...
        return WheelArchiveFile(self._zip.open(archive_name), archive_name, record_entry)
...
class WheelWriter:
    ...
    def __repr__(self) -> str:
        return f"{self.__class__.__name__}({self.path_or_fd}, generator={self.generator!r})"
